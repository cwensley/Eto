...
public static class WinUIMenuHelper
{
	...
	static readonly Regex PlatformMnemonic = new(@"(?<=([^_](?:[_]{2})*)|^)[_](?![_])", RegexOptions.Compiled);
	static readonly Regex EtoMnemonic = new(@"(?<=([^&](?:[&]{2})*)|^)[&](?![&])", RegexOptions.Compiled);
	...
	internal static void GetMenuTextAndAccessKey(string? text, out string menuText, out string accessKey)
	{
		...
		var match = EtoMnemonic.Match(text);
		if (match.Success)
		{
			menuText = text.Remove(match.Index, 1).Replace("&&", "&");
			accessKey = text.Substring(match.Index + 1, 1);
		}
		else
		{
			menuText = text.Replace("&&", "&");
			accessKey = string.Empty;
		}
	}
}
